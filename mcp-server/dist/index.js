...
server.setRequestHandler(ListToolsRequestSchema, async () => {
    return {
        tools: [
            {
                name: "search_tweets",
                description: "Search through saved tweets in the Glana swipe file. Can filter by theme, tag, search term, favorites only, or unclassified tweets.",
                inputSchema: {
                    type: "object",
                    properties: {
                        search: {
            ...
            },
            {
                ...
                description: "Get random tweet suggestions for inspiration. Prioritizes favorites. Use this to inspire content creation or find ideas.",
                inputSchema: {
                    type: "object",
                    properties: {
                        themeId: {
            ...
            {
                name: "list_themes",
                description: "List all themes in the Glana swipe file with their tweet counts.",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
            ...
            {
                name: "get_stats",
                description: "Get statistics about the Glana swipe file: total tweets, themes, unclassified count, tweets per theme, top tags, and last capture date.",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
    ...
    try {
        switch (name) {
            case "search_tweets": {
                ...
                if (args?.themeId)
                    params.themeId = String(args.themeId);
                if (args?.tag)
            ...
            case "list_themes": {
                ...
                const formatted = themes
                    .map((t) => `• ${t.name} (${t.tweetCount} tweets)${t.description ? `: ${t.description}` : ""}`)
                    .join("\n");
                return {
                    content: [
                        {
